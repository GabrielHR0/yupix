...
class ServerRegisterManager(models.Manager):
    def create_user(self, **extra_fields):
        user = self.model(**extra_fields)
        user.save(using=self._db)
        return user
...
class ServerRegister(models.Model):
    ...
    confirm_password = models.CharField(max_length=20, verbose_name = "Confirmar Senha", default=None)
    created_at = models.DateTimeField(default = timezone.now)
    document = models.CharField(max_length=10, verbose_name="Documento de identidade", default=None)
    service = models.CharField(max_length=25, verbose_name="Serviço especilizado", default=None)

    objects = ServerRegisterManager()